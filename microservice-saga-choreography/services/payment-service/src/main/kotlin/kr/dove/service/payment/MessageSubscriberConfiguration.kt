...
class MessageSubscriberConfiguration(
    ...
    @Bean
    fun airlineMessageSubscriber(): Consumer<Event<String, Ticket>> {
        return Consumer<Event<String, Ticket>> { (type, key, ticket, publishedAt) ->
            logger.info("Processing message from topic 'airlines' published at {} and event type is {}", publishedAt, type)
            when (type) {
